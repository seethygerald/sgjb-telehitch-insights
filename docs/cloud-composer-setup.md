...
### Continue from the exact macOS errors shown above

On modern macOS, `python` often does not exist but `python3` does. The second error means that Python 3 is working, but the Telethon package has not yet been installed for it.

From the repository directory—the prompt should end with `sgjb-telehitch-insights %`—run this one command:
...
1. Confirm that `python3` exists.
2. Create a project-only virtual environment named `.venv` if needed.
3. Upgrade `pip` inside that virtual environment.
4. Install Telethon inside that virtual environment.
5. Start the StringSession generator using the correct Python executable.

The installation requires an internet connection. When prompted, enter your Telegram API ID, API hash, phone number, Telegram login code, and two-step-verification password if enabled. The API hash and two-step-verification password will not be displayed while you type.

If macOS says `permission denied`, run:

```bash
chmod +x scripts/setup_telegram_session_macos.sh
./scripts/setup_telegram_session_macos.sh
```

If the setup script is not present, update your repository first:
...
If you prefer to execute each command yourself, copy and run these commands one line at a time:
...
python3 -m pip install "telethon>=1.34"
python3 scripts/generate_telegram_string_session.py
```

After `source .venv/bin/activate`, your prompt should usually begin with `(.venv)`. While the virtual environment is active, both `python` and `python3` should use `.venv`.

If virtual-environment creation reports that `venv` or Python is unavailable, install the current Python 3 release from <https://www.python.org/downloads/macos/>, close Terminal, reopen it, return to this repository, and retry.
...
### What happens during Telegram authentication

1. At `Telegram API ID:`, enter the numeric API ID from `my.telegram.org` and press Return.
2. At `Telegram API hash (input hidden):`, paste the API hash and press Return. Nothing appears as you type or paste; this is intentional.
3. If asked for a phone number, enter the full international number, including `+` and country code.
4. Telegram sends a login code, usually in the Telegram application.
5. Enter that code in Terminal.
6. Enter your Telegram two-step-verification password if requested.
7. After authentication, the helper prints a value between the `TELEGRAM_SESSION_STRING START` and `TELEGRAM_SESSION_STRING END` markers.
8. Copy only the long value between those markers into a password manager temporarily.
9. Do not paste that value into source code, GitHub, a CSV, or a shared document.
10. If you used the manual method, run `deactivate` when finished.

### Troubleshooting the two errors

| Error | Meaning | Fix |
|---|---|---|
| `zsh: command not found: python` | macOS has `python3`, but no `python` alias | Use `python3`, or activate `.venv` first |
| `ModuleNotFoundError: No module named 'telethon'` | Telethon is not installed in the Python environment that ran the helper | Run `./scripts/setup_telegram_session_macos.sh` |
| `No module named pip` | That Python installation lacks pip | Reinstall current Python 3 from python.org |
| `CERTIFICATE_VERIFY_FAILED` | Python cannot verify HTTPS certificates | Run the `Install Certificates.command` supplied in the Python Applications folder, then retry |
| `FloodWaitError` | Telegram has temporarily rate-limited login attempts | Stop retrying and wait for the number of seconds reported by Telegram |
| Login code never arrives | Telegram often sends it to the Telegram app rather than SMS | Check the verified Telegram service conversation and your other logged-in devices |
